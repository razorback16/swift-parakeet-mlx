...
public class DwStridingSubsampling: Module {
    ...
    private func convForward(_ x: MLXArray) -> MLXArray {
        ...
        for (i, layer) in conv.enumerated() {
            let beforeMax = x.max().item(Float.self)

            if let convLayer = layer as? Conv2d {
                // Check if this is the first layer and if the weights look like they were loaded
                if i == 0 {
                    let weightSum = convLayer.weight.sum().item(Float.self)
    ...
    public func callAsFunction(_ x: MLXArray, lengths: MLXArray) -> (MLXArray, MLXArray) {
        ...
        let expectedInputSize = convChannels * finalFreqDim

        let weight = out.weight
        let maxVal = weight.max().item(Float.self)
        let minVal = weight.min().item(Float.self)

        if let bias = out.bias {
            let biasMaxVal = bias.max().item(Float.self)
            let biasMinVal = bias.min().item(Float.self)
        }
...
public class LocalRelPositionalEncoding: RelPositionalEncoding {
    let leftContext: Int
...
public class Conformer: Module {
    ...
    public func callAsFunction(
        ...
        for (i, (layer, cache)) in zip(layers, actualCache).enumerated() {
            let xBefore = x.max().item(Float.self)
...
private func floor(_ x: MLXArray) -> MLXArray {
    return MLX.floor(x)
}

private func sin(_ x: MLXArray) -> MLXArray {
    return MLX.sin(x)
}
...
private func exp(_ x: MLXArray) -> MLXArray {
    return MLX.exp(x)
}

private func log(_ x: Float) -> Float {
